#!/usr/bin/env python3
"""Differential isolated settings transport characterization: same test class, two separate JVMs."""
import argparse
import json
import os
...
for label,jar in [('baseline',ROOT/'build/baseline/HubPilot-Core-1.0.2.jar'),('reconstructed',candidate)]:
 ...
 r=subprocess.run([str(a.jdk/'bin/java'),'-Xverify:all','-cp',cp,'dev.hubpilot.core.SettingsSyncCharacterization'],capture_output=True,text=True,timeout=90)
 (ROOT/f'build/{label}-settings.log').write_text(r.stdout+r.stderr)
 print(label+':\n'+r.stdout+r.stderr,flush=True)
 if r.returncode: raise SystemExit(r.returncode)
 outputs.append(r.stdout)
assert outputs[0]==outputs[1], 'Differential settings transport results differ'
print('PASS identical baseline/reconstruction settings transport observations (reflective event fixtures and actual binary settings framing)')
