...
public class InteractValidation {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            dev.hubpilot.hub.publicapi.AdminItemManager.toggle(hub,p);
            editor.command(p, new String[]{"tool", "on"});
            check(Arrays.stream(p.getInventory().getContents()).filter(Objects::nonNull).count() == 3, "tools not granted");
            editor.command(p, new String[]{"tool", "on"});
            check(Arrays.stream(p.getInventory().getContents()).filter(Objects::nonNull).count() == 3, "duplicate brush");
            editor.command(p, new String[]{"tool", "off"});
            check(p.getInventory().getItem(3).getAmount() == 12, "ordinary inventory changed");
            check(Arrays.stream(p.getInventory().getContents()).anyMatch(i -> MenuItems.isAdminOpener(hub,i)), "Interact changed Hub item");
            editor.command(p,new String[]{"items","admin","off"});
            check(Arrays.stream(p.getInventory().getContents()).anyMatch(i -> MenuItems.isAdminOpener(hub,i)), "legacy Interact admin route still active");
            dev.hubpilot.hub.publicapi.AdminItemManager.toggle(hub,p);
            ...
            var guest = server.addPlayer(); editor.command(guest,new String[]{"tool"});
            check(Arrays.stream(guest.getInventory().getContents()).noneMatch(Objects::nonNull), "unauthorized grant");
            System.out.println("PASS portal persistence, brush-only toggle, Hub item isolation, permissions, deduplication and inventory safety");
